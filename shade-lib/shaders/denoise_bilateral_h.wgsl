// Horizontal joint bilateral filter pass, operating in linear YCbCr (BT.709).
//
// The range weights are computed from a pre-blurred guide image rather than the
// noisy input — this stabilises the weights and avoids the blocky artefacts that
// naive bilateral produces on high-noise images.
//
// Luma and chroma channels are filtered independently with per-channel sigma_r
// derived from luma_strength / chroma_strength (0–1 user sliders).
...
// σ_s = 2.5, radius = 5, 11-tap Gaussian spatial kernel
