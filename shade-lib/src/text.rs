...
/// Bundled default font (Inter Regular, SIL OFL). Used as a fallback when a
/// `Layer::Text` references a font that has not been uploaded — so the user
/// always sees *something* render. See `LayerStack::ensure_default_font`.
pub const DEFAULT_FONT_BYTES: &[u8] = include_bytes!("../assets/Inter-Regular.ttf");

/// Family label registered for [`DEFAULT_FONT_BYTES`].
