...
pub async fn add_text_layer(
    ...
    let idx = {
        let mut st = lock_editor_state(&state)?;
        let mut style = shade_lib::TextStyle::new(params.font_id, params.size_px);
...
fn deser_double_option_f32<'de, D>(d: D) -> Result<Option<Option<f32>>, D::Error>
where
...
fn parse_text_align(s: &str) -> Result<shade_lib::TextAlign, String> {
    Ok(match s {
...
fn parse_text_anchor(s: &str) -> Result<shade_lib::TextAnchor, String> {
    Ok(match s {
...
#[tauri::command]
pub async fn set_text_transform(
...
#[tauri::command]
pub async fn prune_unused_fonts(
    state: tauri::State<'_, Mutex<EditorState>>,
) -> Result<usize, String> {
    let removed = {
        let mut st = lock_editor_state(&state)?;
...
fn text_align_str(a: shade_lib::TextAlign) -> &'static str {
    match a {
...
fn text_anchor_str(a: shade_lib::TextAnchor) -> &'static str {
    match a {
