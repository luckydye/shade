...
export type CurveEditorBaseProps = {
  ...
  normalizeInteriorPoint: (pt: ControlPoint) => ControlPoint;
  buildSvgCurvePath: (lut: readonly number[]) => string;
  /** Y-axis range: 1 for tone curve (0–1), 2 for LS curve (0–2) */
  yMax: 1 | 2;
  endpointResetY: (point: EditableControlPoint) => number;
  bottomLabels?: [string, string];
  showToneGuide?: boolean;
};

export const CurveEditorBase: Component<CurveEditorBaseProps> = (props) => {
  ...
  const hoveredToneGuide = createMemo(() => {
    if (!props.showToneGuide) return null;
  ...
  const svgCoords = (event: { clientX: number; clientY: number }) => {
    ...
    return {
      ...
      y: clamp(yMax * (1 - (local.y - graphPadding) / innerHeight()), 0, yMax),
    };
  };
  ...
  const updateDraggingPoint = (clientX: number, clientY: number) => {
    ...
    if (!current) throw new Error("dragged curve point not found");
    const nextCoords = svgCoords({ clientX, clientY });
    const { x, y } = isEndpointPoint(current)
      ? { x: current.x, y: clamp(nextCoords.y, 0, props.yMax) }
      : props.normalizeInteriorPoint(nextCoords);
    const next = pts()
  ...
  const startNewPointDrag = (clientX: number, clientY: number) => {
    const { x, y } = props.normalizeInteriorPoint(svgCoords({ clientX, clientY }));
    const id = nextId++;
  ...
  const startExistingPointDrag = (id: number) => {
    ...
    if (!point) throw new Error("curve point not found");
    setHoveredId(id);
    const now = Date.now();
  ...
  const midLinePath = () => {
    const p = graphPadding;
    const w = innerWidth();
    const h = innerHeight();
    if (props.yMax === 1) {
      return `M ${p} ${p + h} L ${p + w} ${p}`;
    }
    return `M ${p} ${p + h * 0.5} L ${p + w} ${p + h * 0.5}`;
  };
  ...
  return (
    ...
    >
      ...
      <div class="col-start-2 col-end-4 overflow-hidden">
            ...
            <>
              ...
              <circle
                ...
                onTouchStart={(event) => {
                  ...
                  event.preventDefault();
                  if (!startExistingPointDrag(point.id)) return;
                  trackTouchDrag(touch.identifier);
                }}
                onPointerDown={(event) => {
        ...
        <Show when={props.bottomLabels}>
          {(labels) => (
            <div class="mt-1 flex justify-between px-1">
              <span class="text-[10px] text-[var(--text-faint)]">{labels()[0]}</span>
              <span class="text-[10px] text-[var(--text-faint)]">{labels()[1]}</span>
            </div>
          )}
        </Show>
      </div>
