    ...
    fn get_cache_directory() -> Result<PathBuf, FileLoaderError> {
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            if let Some(cache_dir) = dirs::cache_dir() {
                ...
                Ok(home_dir.join(".cache").join("shade").join(Self::CACHE_SUBDIR))
            } else {
                ...
                match bincode::deserialize::<CachedImage>(&data) {
                    Ok(cached_image) => {
                        ...
                        if cached_image.cache_version == Self::CACHE_VERSION {
                            ...
                            log::info!("Cache version mismatch for key: {}, removing old cache", cache_key);
                    ...
                    Err(e) => {
                        log::warn!("Failed to deserialize cache file {}: {}", cache_path.display(), e);
    ...
    pub fn clear_cache(&self) -> Result<(), FileLoaderError> {
        if self.cache_dir.exists() {
            ...
            fs::create_dir_all(&self.cache_dir).map_err(|e| {
                FileLoaderError::IoStringError(format!("Failed to recreate cache directory: {}", e))
            })?;
        }
