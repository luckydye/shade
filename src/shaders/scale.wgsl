...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Apply inverse scaling to find source coordinates
    let center = vec2<f32>(0.5, 0.5);
    let offset_coords = normalized_coords - center;
    let scaled_coords = offset_coords / params.factor + center;

    // Convert back to texture coordinates
    let source_coords = scaled_coords * vec2<f32>(f32(input_dimensions.x), f32(input_dimensions.y));
