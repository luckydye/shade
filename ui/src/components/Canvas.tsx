...
function getDisplayBounds(
	...
	const scale =
		Math.min(stageWidth / crop.width, stageHeight / crop.height) *
		state.previewZoom;
	return {
...
const Canvas: Component = () => {
	...
	function cropHandleAtPoint(x: number, y: number) {
		...
		const bounds = getFullImageBounds(
			stageRef.clientWidth,
			stageRef.clientHeight,
		);
	...
	const onPointerDown = (e: PointerEvent) => {
		...
		if (selectedCropLayer()) {
			const rect = stageRef.getBoundingClientRect();
			const handle = cropHandleAtPoint(
				e.clientX - rect.left,
				e.clientY - rect.top,
			);
			if (!handle) return;
			...
			case "move":
				next = {
					...
					x: Math.min(
						...
						state.canvasWidth - start.width,
					),
					y: Math.min(
						Math.max(0, start.y + deltaY),
						state.canvasHeight - start.height,
					),
				};
				break;
			case "top-left":
						...
						<div class="pointer-events-none absolute flex max-w-sm flex-col items-center gap-3 rounded-[26px] border border-white/8 bg-black/40 px-8 py-10 text-center backdrop-blur-sm">
							...
							<div>
								...
								<div class="mt-1 text-sm text-white/48">
									Drag a photo into the stage or use the Open action in the top
									bar.
								</div>
							</div>
						</div>
