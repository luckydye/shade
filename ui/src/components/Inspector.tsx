...
function buildLuminanceHistogram(frame: ImageData, binCount = 64) {
	...
	for (let idx = 0; idx < data.length; idx += 4) {
		...
		const binIdx = Math.min(
			binCount - 1,
			Math.floor(luminance * (binCount - 1)),
		);
		bins[binIdx] += 1;
	}
	return bins;
...
const Inspector: Component = () => {
	...
	const curves = () =>
		selectedAdjustmentLayer()?.adjustments?.curves ?? DEFAULT_CURVES;
	const color = () =>
		selectedAdjustmentLayer()?.adjustments?.color ?? DEFAULT_COLOR;
	const vignette = () =>
		...
		createEffect(
			on(
				...
				(layerIdx) => {
					...
					nextId = 0;
					setPts(
						(points.length === 0 ? defaultCurvePoints() : points).map(
	...
	const PresetsPanel: Component = () => (
		<div class="flex flex-col gap-4 pt-1">
			...
			<Show when={presetStatus()}>
				{(status) => (
					<div class="text-[11px] font-medium text-white/45">{status()}</div>
				)}
			</Show>
				...
				>
					{presets().map((preset) => (
						<div class="flex items-center gap-2 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2.5">
							<div class="min-w-0 flex-1 truncate text-[13px] font-semibold text-white/80">
								{preset.name}
							</div>
							<button
								type="button"
					...
					{inspectorTab() === "presets" ? (
						...
						<Show
							...
							fallback={
								<Show
									when={
										state.selectedLayerIdx >= 0 && selectedAdjustmentLayer()
									}
									fallback={
			...
			>
				...
				<div class="flex items-center gap-1 overflow-x-auto border-t border-white/6 px-4 pb-[calc(env(safe-area-inset-bottom)+2rem)] pt-3">
					{adjustmentLayers().map(({ idx }) => {
						...
						const isActive = () =>
							state.selectedLayerIdx === idx && isDrawerOpen();
						return (
							<button
								type="button"
								onClick={() => {
