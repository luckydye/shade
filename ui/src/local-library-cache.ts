...
async function loadLocalLibraryListing(
	libraryId: string,
): Promise<LibraryImageListing> {
	return withStores([LIBRARIES_STORE, ITEMS_STORE], "readonly", async (stores) => {
		const library = (await requestToPromise(
			stores[LIBRARIES_STORE].get(libraryId),
		)) as CachedLocalLibrary | undefined;
...
async function saveLocalLibraryListing(
	...
	await withStores([LIBRARIES_STORE, ITEMS_STORE], "readwrite", async (stores) => {
		...
		await Promise.all(
			keys
				.filter(
					(key) => typeof key === "string" && key.startsWith(`${libraryId}:`),
				)
