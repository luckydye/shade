import { getPeerThumbnailBytes, listPeerPictures, type SharedPicture } from "./bridge/index";
...
async function getCachedThumbnail(peerId: string, pictureId: string): Promise<Blob | null> {
  return withStore("readonly", async (store) => {
    const result = await requestToPromise(store.get(thumbnailKey(peerId, pictureId)));
    return result instanceof Blob ? result : null;
  });
}

async function putCachedThumbnail(peerId: string, pictureId: string, blob: Blob): Promise<void> {
...
function toPeerLibraryItems(peerId: string, pictures: SharedPicture[]): PeerLibraryItem[] {
  ...
  async function worker() {
    while (nextIndex < pictures.length) {
      ...
      try {
        ...
        await putCachedThumbnail(peerId, picture.id, new Blob([toBlobBuffer(bytes)], { type: "image/jpeg" }));
      } catch {
        return;
      }
    }
...
export async function loadPeerLibraryItems(peerId: string): Promise<PeerLibraryItem[]> {
  try {
    const pictures = await listPeerPictures(peerId);
    persistPeerLibraryItems(peerId, pictures);
    void warmPeerLibraryThumbnails(peerId, pictures);
