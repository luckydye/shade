...
function getPreviewCropSize(zoom: number) {
	...
	return {
		width: Math.min(
			previewScale.bounds.width,
			state.previewViewportWidth / imageScale,
		),
		height: Math.min(
...
function getVisiblePreview(zoom: number, centerX: number, centerY: number) {
	...
	const imageScale = previewScale.fitScale * zoom;
	const imageX =
		state.previewViewportWidth * 0.5 -
		(center.x - previewScale.bounds.x) * imageScale;
	const imageY =
		state.previewViewportHeight * 0.5 -
		(center.y - previewScale.bounds.y) * imageScale;
...
export function getMaxPreviewZoom() {
	...
	return Math.max(1, 1 / previewScale.fitScale);
}
